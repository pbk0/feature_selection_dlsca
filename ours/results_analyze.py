...
def test_fifty_random_model_runs(_exp_type: str):
    
    # PdfPages is a wrapper around pdf
    # file so there is no clash and create
    # files with no error.
    _pdf_file = pathlib.Path(f"_results/opoi_{_exp_type}_test_fifty_random_model_runs.pdf")
    ...
    with PdfPages(_pdf_file) as _pdf:
        for _ds in [
            ...
            _fig = test_fifty_random_model_runs_for_dataset(_dataset=_ds, _exp_type=_exp_type)
...
def test_fifty_random_model_runs_for_dataset(_dataset: str, _exp_type: str, ) -> plt.Figure:
    ...
    for _npz in pathlib.Path(f"_results/{_dataset}/opoi/{_exp_type}/test_fifty_random_model_runs").glob("*"):
        if not _npz.name.endswith(".npz"):
            continue
...
def main():
    ...
    elif _mode == "best_model_runs":
        __exp_type = sys.argv[2]
        best_model_runs(_exp_type=__exp_type)
    elif _mode == "fifty_random_model_runs":
        __exp_type = sys.argv[2]
        best_model_runs(_exp_type=__exp_type)
    else:
